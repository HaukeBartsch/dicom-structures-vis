...
// Request the data
d3.json('data/dataDICOM2.json', function (err, response) {

    var svg, events, charactersMap, width, height, sceneWidth;

    // Get the data in the format we need to feed to d3.layout.narrative().scenes
    events = wrangle(response);

    // Some defaults
    sceneWidth = 10;
    width = events.length * sceneWidth * 4 + 500;
    height = 600;
    labelSize = [150, 15];
    ...
    events.forEach(function (scene) {
        scene.participants.forEach(function (participant) {
            participant.width = svg.append('text')
                .attr('opacity', 0)
                .attr('class', 'temp')
                .text(participant.patientid)
    ...
    svg.selectAll('.link').data(narrative.links()).enter()
        ...
        .attr('class', function (d) {
            return 'link ' + d.participant.affiliation.toLowerCase();
        })
        .attr('d', narrative.link());


    // Draw the scenes
    svg.selectAll('.scene').data(narrative.events()).enter()
        .call(function (s) {


            g = s.append('g').attr('class', 'scene')
            ...
            g.append('g')
                ...
                .text(function (d) {
                    var event_label = d.event.event;
                    var series_number = d.event.seriesnumber;
                    return event_label + " " + series_number.padStart(3, '0');
                })
                .attr('x', '0px')
                .attr('y', '-3px');
        });
    ...
    svg.selectAll('.scene').selectAll('.appearance').data(function (d) {
        ...
        .attr('class', function (d) {
            return 'appearance ' + d.participant.affiliation;
        });
    ...
    svg.selectAll('.intro').data(narrative.introductions())
        .enter().call(function (s) {
            ...
            g.selectAll('text')
                ...
                .attr('x', '-8px')
                .text(function (d) {
                    return d.participants.patientid;
                });

            g.select('.color')
                .attr('class', function (d) {
                    return 'color ' + d.participants.affiliation;
                });

            g.select('rect')
                .attr('class', function (d) {
                    return d.participants.affiliation;
                });
...
function wrangle(data) {
    ...
    return data.events.map(function (event) {
        return {
            participants: event.participants.map(function (id) {
                return characterById(id);
            }).filter(function (d) { return (d); }),
            event: { 'event': event.event, 'seriesnumber': event.seriesnumber }
        };
    });
    ...
    function characterById(id) {
        ...
        charactersMap[id] = charactersMap[id] || data.participants.find(function (participant) {
