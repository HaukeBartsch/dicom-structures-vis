...
d3.layout.narrative = function () {
	...
	var narrative,
		events, participants, introductions, links,
	...
	// ----------
	//
	// `narrative.participants([array])`
	//
	// Set or get the participants array. If an array is passed, sets the
	// narrative's participants array, otherwise returns the participants array.
	narrative.participants = function (_) {
		if (!arguments.length) {
			return participants;
		}
		participants = _;
	...
	narrative.extent = function () {
		return events.concat(introductions).reduce(function (max, d) {
			var bounds = d.bounds();
			if (bounds[1][1] > max[1]) {
				max[1] = bounds[1][1];
			}
	...
	//
	// The participants are divided into groups based on the strength of their relationships
	...
	// ----------------------
	//
	// Populate the scenes with participants from the participants array.
	// This method also cleanses the data to exclude participants which appear only once
	// and scenes with fewer than two participants.
	function computeSceneParticipants() {

		var appearances, finished;

		// Create a map of scenes to participants (i.e. appearances).
		appearances = [];
		events.forEach(function (scene) {
			scene.participants.forEach(function (character) {

				// If the character isn't an object assume it's an index from the participants array.
				character = (typeof character === 'object') ? character : participants[character];
				...
				appearances.push({ participant: character, scene: scene });
		...
		// Recursively filter appearances so we ultimately only include participants
		...
		function filterAppearances(appearance) {
			...
			counts = appearances.reduce(function (c, a) {
				...
				if (appearance.participant === a.participant) {
					c[0]++;
				}
		...
		// Re-construct `participants` and `scenes` arrays with filtered appearances.
		participants = [];
		events = [];
		appearances.forEach(function (appearance) {

			// Cross reference scenes and participants based on appearances.
			appearance.scene.appearances.push(appearance);
			appearance.participant.appearances.push(appearance);

			if (participants.indexOf(appearance.participant) === -1) {
				participants.push(appearance.participant);
			}

			if (events.indexOf(appearance.scene) === -1) {
				events.push(appearance.scene);
			}
		});
	}
	...
	// --------------------
	//
	// Cluster participants based on their co-occurence in scenes
	function computeCharacterGroups() {
		var nodes, edges, clusters, partitioner, groupsMap, initGroups;

		// An array of character indexes.
		nodes = participants.map(function (d, i) { return i; });

		initGroups = participants.reduce(function (g, d, i) {
			if (d.initialgroup) {
				g[i] = +d.initialgroup;
			}
			return g;
		...
		// Put all participants in groups with bi-directional reference.
		groups = [];
		groupsMap = {};
		participants.forEach(function (character, i) {
			var groupId, group;
			groupId = clusters[i];
			...
			group.participants.push(character);
			character.group = group;
		});

		// Creates a single link between each pair of participants in a scene.
		function sceneEdges(list) {
			var i, j, matrix;
			matrix = [];
			for (i = list.length; i--;) {
				for (j = i; j--;) {
					matrix.push([participants.indexOf(list[i].participant), participants.indexOf(list[j].participant)]);
				}
			}
			return matrix;
		}
	...
	// participants appearing in that scene.
	...
	function setSceneGroups() {
		events.forEach(function (scene) {
			...
			scene.appearances.forEach(function (appearance) {
				...
				index = groups.indexOf(appearance.participant.group);
	...
	// -----------------
	//
	// Assign unique set of participants to each group based on appearances in
	// scenes belonging to that group.
	function computeGroupAppearances() {
		events.forEach(function (scene) {
			var participants;
			participants = scene.appearances.map(function (a) {
				return a.participant;
			});
			scene.group.appearances = scene.group.appearances || [];
			scene.group.appearances = scene.group.appearances.concat(participants.filter(function (character) {
	...
	function computeCharacterGroupPositions() {
		...
		groups.forEach(function (group) {
			group.participants.sort(function (a, b) {
				var diff;
	...
	// of all participants which appear in scenes assigned to this group.
	...
	// -------------------
	//
	// Compute the position of participants within a scene.
	function computeAppearancePositions() {
		...
		events.forEach(function (scene) {
			...
			scene.appearances.sort(function (a, b) {
				...
				// Try simple group order.
				diff = a.participant.group.order - b.participant.group.order;
				if (diff !== 0) {
					return diff;
				}

				// For participants in the same group use average scene position.
				diff = a.participant.averageScenePosition - b.participant.averageScenePosition;
				if (diff !== 0) {
					return diff;
				}

				// All else failing use main participants array order to keep things consistent.
				return participants.indexOf(a.participant) - participants.indexOf(b.participant);
			});
	...
	function computeScenePositions() {
		...
		events.forEach(function (scene) {
			...
			appearances = scene.appearances.filter(function (appearance) {
				return appearance.participant.group !== scene.group;
			});
			...
			sum = appearances.reduce(function (total, appearance) {
				return total += characterPosition(scene.group.appearances.indexOf(appearance.participant)) + scene.group.min;
			}, 0);
	...
	function createIntroductionNodes() {
		...
		appearances = participants.map(function (character) {
		...
		appearances.forEach(function (appearance) {
			...
			introduction = {
				participants: appearance.participant,
			...
			if (orientation === 'vertical') {
				introduction.x = appearance.participant._x || Math.max(0 + labelSize[0] / 2, Math.min(size[0] - labelSize[0] / 2, x));
				introduction.y = appearance.participant._y || Math.max(0, Math.min(size[1] - labelSize[1], y));
			} else {
				introduction.x = appearance.participant._x || Math.max(0, Math.min(size[0] - labelSize[0], x));
				introduction.y = appearance.participant._y || Math.max(0 + labelSize[1] / 2, Math.min(size[1] - labelSize[1] / 2, y));
			}

			introduction.width = appearance.participant._width || labelSize[0];
			introduction.height = appearance.participant._height || labelSize[1];

			appearance.participant.introduction = introduction;
	...
	function computeIntroductionPositions() {
		...
		collidables = introductions.concat(events);
		...
		function resolveCollisionsHorizontal(introduction) {
			...
			movable = collisions.filter(function (collision) { return (collision.participants); });
			movable.forEach(moveCollision);

			// Now only consider immovables (i.e. scene nodes).
			collisions = collisions.filter(function (collision) { return !(collision.participants); });
		...
		function resolveCollisionsVertical(introduction) {
			...
			// Move colliding items out of the way if possible.
			movable = collisions.filter(function (collision) { return (collision.participant); });
			movable.forEach(moveCollision);

			// Now only consider immovables (i.e. scene nodes).
			collisions = collisions.filter(function (collision) { return !(collision.participant); });
	...
	function characterPosition(index) {
		return index * (1 * pathSpace) + pathSpace / 2;
	}
